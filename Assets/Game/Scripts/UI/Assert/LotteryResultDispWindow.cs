...
/// </summary>
public class LotteryResultDispWindow : Window
{
    ...
    {
        WindowManager.Instance.Show<LotteryResultDispWindow>(false);
    }
    ...
    public void Refresh(SCLottery lottery)
    {
        ...
        {
            long uuid = Convert.ToInt64(lottery.RewardItem[0].Uuid);
            var info = HeroModelLocator.Instance.FindHero(uuid);
            RefreshHero(info);
        }
        else if (lottery.LotteryType == LotteryConstant.LotteryTypeItem)
        {
            var info = ItemModeLocator.Instance.FindItem(lottery.RewardItem[0].Uuid);
            RefreshItem(info);
        }
    }

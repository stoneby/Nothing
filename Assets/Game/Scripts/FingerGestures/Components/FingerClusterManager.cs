...
public class FingerClusterManager : MonoBehaviour 
{
	...
	public void Update()
    {
        ...
        foreach( FingerGestures.Finger finger in fingersRemoved )
        {
            ...
            for( int i = clusters.Count - 1; i >= 0; --i )
            {
                ...
                if( cluster.Fingers.Remove( finger ) )
                {
                    ...
                    {
                        //Logger.Log( "Recycling cluster " + cluster.Id );
        ...
        foreach( FingerGestures.Finger finger in fingersAdded )
        {
            ...
            if( cluster == null )
            {
                ...
                //Logger.Log( "Created NEW cluster " + cluster.Id + " for " + finger );                
            }
            else
            {
                //Logger.Log( "Found existing cluster " + cluster.Id + " for " + finger );
            }
    ...
    Cluster NewCluster()
    {
        ...
        //Logger.Log( "Created new finger cluster #" + cluster.Id );
        return cluster;
    }

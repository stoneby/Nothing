...
public abstract class GestureRecognizer<T> : GestureRecognizer where T : Gesture, new()
{
    ...
    {
        //Log.I( this.GetType().Name + " changed state from " + gesture.PreviousState + " to " + gesture.State );
    }
    ...
    {
        //Log.I( "Beginning " + this.GetType().Name );
        ...
        if( gesture.Fingers.Count > 0 )
            Logger.LogWarning( this.name + " begin gesture with fingers list not properly released" );
#endif
    ...
    protected virtual void UpdateGesture( T gesture, FingerGestures.IFingerList touches )
    {
        ...
        switch( gesture.State )
        {
            ...
            default:
                Logger.LogError( this + " - Unhandled state: " + gesture.State + ". Failing gesture." );
